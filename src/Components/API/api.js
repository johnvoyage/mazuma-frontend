...
const allUsersAccounts = userId => {
  return fetch(`${API_ROOT}/users/${userId}/accounts/`).then(response =>
    response.json()
  );
};

const allUsersEntries = userId => {
  return fetch(`${API_ROOT}/users/${userId}/entries/`).then(response =>
    response.json()
  );
};

const allEntrysTransactions = entryId => {
  return fetch(`${API_ROOT}/entries/${entryId}/transactions/`).then(response =>
    response.json()
  );
};

const readAccount = accountId => {
  return fetch(`${API_ROOT}/accounts/${accountId}/`).then(response =>
    response.json()
  );
};
